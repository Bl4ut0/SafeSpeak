...
/// plays it through a dedicated private audio router. The bounded latest-wins
/// queue prevents rapid keyboard navigation from building a speech backlog.
/// </summary>
public sealed class EnhancedReaderSpeechOutput : IReaderSpeechOutput, IAsyncDisposable
{
    ...
    public EnhancedReaderSpeechOutput(ITtsEngine ttsEngine, IAudioRouter audioRouter)
    {
    ...
    private async Task ProcessRequestsAsync()
    {
        try
        {
            await foreach (SpeechRequest initialRequest in _requests.Reader.ReadAllAsync(_lifetimeCts.Token))
            {
                ...
                while (_requests.Reader.TryRead(out SpeechRequest? newerRequest))
                {
                ...
                {
                    // Expected when focus advances, speech is interrupted, or the app closes.
                }
                catch
                {
