...
- TikFinity connection and reconnection through a platform-neutral source-connector contract. An experimental **TikTok Direct (test)** choice in Settings connects to a public LIVE stream by creator username without TikFinity or a hosted signing service. SafeSpeak remains disarmed after connecting or changing sources.
...
- The bundled 23M-parameter MiniLM classifier is a specialized local text model, not a guarantee. Deterministic rules and a heuristic fallback remain active around it. Qwen3Guard is optional, uses about 484 MB of local storage plus additional CPU and memory, and is reported as active only after SafeSpeak's private service returns a valid native safety response.
...
- TikTok Direct is an unofficial compatibility test. Its parser, bounds, source switching, and shutdown behavior have automated coverage, but each TikTok protocol change can require an update and live event coverage still needs validation against an active stream.
