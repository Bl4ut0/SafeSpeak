...
public sealed class PrivateVoicePreviewOutputTests
{
    [Fact]
    public async Task Speak_UsesConfiguredStreamVoiceAndPrivateRouter()
    {
        var engine = new RecordingTtsEngine();
        var router = new RecordingAudioRouter();
        await using var output = new PrivateVoicePreviewOutput(engine, router)
        {
