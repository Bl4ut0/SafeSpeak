...
The active voice and volume are shared by moderated stream TTS and SafeSpeak's optional integrated reader. Reader speed has its own Accessibility-tab slider and defaults to a faster navigation rate. **Test selected voice** plays a private sample even when the integrated reader is disabled. Reader guidance and voice tests use the private-monitor device when that route is enabled, or the Windows default output otherwise; they are never sent to the broadcast route.

Kokoro inference is deliberately serialized because running multiple copies of the model can create severe CPU and memory pressure. Keyboard-focus phrases use a bounded memory cache plus a privacy-limited 64 MB on-disk cache. On the first visit to an uncached control, SafeSpeak speaks immediately with the low-latency Windows voice while generating the selected neural phrase in the background. Later visits—including after an app restart for non-sensitive static controls—play the selected neural voice immediately. Dynamic list items, edit fields, and device selections are not persisted. Windows Narrator, NVDA, and JAWS remain independent and use their own configured voices through UI Automation.
