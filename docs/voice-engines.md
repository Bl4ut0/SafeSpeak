...
The active voice, stream rate, and volume apply to moderated TikTok TTS and **Test selected voice**. The test plays a private sample even when the integrated reader is disabled. It uses the private-monitor device when that route is enabled, or the Windows default output otherwise, and is never sent to the broadcast route.

SafeSpeak's integrated reader deliberately uses a separate low-latency Windows voice for keyboard focus and application status. Its speed has an independent Accessibility-tab slider. Kokoro is not invoked while tabbing, so navigation does not wait for neural inference and the same control is never announced by both the fast and neural voices. Windows Narrator, NVDA, and JAWS remain independent and use their own configured voices through UI Automation.
